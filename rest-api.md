...
showPermissionSets|BOOLEAN|No|Controls whether the content of the `permissionSets` field is populated or not. Defaults to `false`, unless the `permissions` parameter is set.
...
* If `permissions` parameter not provided, all symbols that have either `SPOT`, `MARGIN`, or `LEVERAGED` permission will be exposed.
  ...
  * You can also opt to use `showPermissionSets=true`. This can be used in combination with the other parameters.
